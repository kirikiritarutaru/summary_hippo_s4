...
## S4 のパラメータの訓練
パラメータ行列 $A$ がユニタリ行列によって対角化されるなら、話が早い。（SSMの計算が早い）
HiPPO 行列はそうではないので3つの工夫を加えることにより、ボトルネックを解消する。

1. FFTを用いた畳み込みの計算量削減
2. Woodbury の公式を用いた計算量削減
3. 対角行列の場合に、 Cauchy カーネルの計算と等価であることを用いた計算量削減
