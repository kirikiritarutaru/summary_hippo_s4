...
## [定義] Lipschitz の局所連続条件
関数 $f(x,y)$ が次の2つの条件をみたすとき、**Lipschitz 連続**であるという。
1. 領域 $D = \{ (x,y) ; | x-x_0|\le a,\ |y-y_0|\le b\}$ で連続である。ただし、$a>0,\ b>0$ 。
2. 領域 $D$ 内の2点 $(x,y_1), (x,y_2)$ での関数値の差が、ある正の数 $K$ を用いて、
    $$
...
### [確かめ] $C^1$ 級の関数であれば、Lipschitz 条件をみたすか？
$f(x,y)$ が $C^1$ 級ならば、$y$ 方向偏導関数 $f_y (x,y)$ も連続だから、その絶対値の上限を $|f_y (x,y)| \le K$ とすると、
$$
f(x,y_2)-f(x,y_1) = \int^{y_2}_{y_1} f_y (x,y)dy \quad  \text{なので}\\
\left|f(x,y_2)-f(x,y_1) \right| \le \left| \int^{y_2}_{y_1} |f_y (x,y)| dy \right| \le K|y_2 -y_1|
$$
となって、Lipschitz 条件を満たす。